...
class rawLoader():
    def getUniqueRecodes(self, _data_dir):
        df = pd.DataFrame(index=[], columns=['master_id','market','link','price','name','date','datetime','stock'])
        ...
        df = df[~df.duplicated(subset=['master_id','market','date','name','link'],keep='first')]
        return df
